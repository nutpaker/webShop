...
function initFormsMailChimp() {
    ...
    $.validator.addMethod('mc_birthday', function (date, element, groupingClass) {
        let isValid = false;
    ...
    $.validator.addMethod('mc_date', function (date, element, groupingClass) {
        let isValid = false;
        const $fields = $('input:not(:hidden)', $(element).closest(groupingClass));
        if ($fields.filter(':filled').length === 0 && this.optional(element)) {
            isValid = true; // None have been filled out, so no error
    ...
    $.validator.addMethod('mc_phone', function (phoneNumber, element, groupingClass) {
        let isValid = false;
        const $fields = $('input:filled:not(:hidden)', $(element).closest(groupingClass));
    ...
    $.validator.addMethod('skip_or_complete_group', function (value, element, groupingClass) {
        const $fields = $('input:not(:hidden)', $(element).closest(groupingClass));
        ...
        const isValid = numberFilled === 0 || numberFilled === $fields.length;
        ...
        const validator = $form.validate({
            ...
            groups() {
                ...
                $form.find('.input-group').each(function () {
                    ...
                    const inputs = $(this).find('input:text:not(:hidden)');
                    if (inputs.length > 1) {
